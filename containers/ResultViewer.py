...
class ResultViewer:
    ...
    def parseResultDetails(self):
        ...
        # Safely handle average CPU usage
        cpu_usage = result['resource']['cpu']
        if isinstance(cpu_usage, float) and (cpu_usage != cpu_usage):  # Check if NaN
            cpu_display = 'N/A'  # or some default value like 0%
        else:
            cpu_display = '{}%'.format(round(cpu_usage, 2))
        
        self.parent.lblCPU.setText('Average CPU Usage: {}'.format(cpu_display))
        
        # Safely handle average memory usage
        memory_usage = result['resource']['memory']
        if isinstance(memory_usage, float) and (memory_usage != memory_usage):  # Check if NaN
            memory_display = 'N/A'  # or some default value like 0
        else:
            memory_display = '{} MB'.format(round(memory_usage, 2))
    
        self.parent.lblMemory.setText('Average Mem Usage: {}'.format(memory_display))
